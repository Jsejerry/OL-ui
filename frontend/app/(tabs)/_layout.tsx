...
function BottomBar({ state, navigation }: any) {
  ...
  return <View testID="bottom-navigation" style={[styles.bar, { paddingBottom: Math.max(insets.bottom, 9) }, dark && styles.dark]}>
    {visible.map(tab => <Pressable key={tab.name} testID={`tab-${tab.name === 'index' ? 'home' : tab.name}`} accessibilityRole="tab" accessibilityLabel={tab.name === 'reels' ? 'Product reels' : tab.label} accessibilityState={{ selected: active === tab.name }} onPress={() => navigation.navigate(tab.name)} style={({ pressed }) => [styles.item, pressed && styles.pressed]}>
      {tab.name === 'reels' ? <View style={[styles.center, dark && styles.darkCenter]}><BrandMark large /></View> : <Icon name={tab.icon as any} size={22} color={active === tab.name ? colors.forest : dark ? colors.onSurfaceInverse : colors.muted} />}
...
export default function TabsLayout() {
  return <Tabs tabBar={props => <BottomBar {...props} />} screenOptions={{ headerShown: false, lazy: true }}>
    {visible.map(t => <Tabs.Screen key={t.name} name={t.name} options={{ title: t.label }} />)}
    {['cart', 'account', 'grocery', 'pharmacy', 'beauty'].map(name => <Tabs.Screen key={name} name={name} options={{ href: null }} />)}
  </Tabs>;
}
const styles = StyleSheet.create({
  bar: { flexDirection: 'row', paddingTop: 11, backgroundColor: colors.surface, borderTopWidth: 1, borderColor: colors.border },
  item: { flex: 1, minHeight: 49, alignItems: 'center', justifyContent: 'flex-start', gap: 5 }, label: { fontSize: 10, fontWeight: '500', color: colors.muted }, active: { color: colors.forest, fontWeight: '700' },
