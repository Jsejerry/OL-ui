...
export function ProductCard({ product: p, width = 158, scope = '' }: { product: Product; width?: number | null; scope?: string }) {
  ...
  return <View style={[styles.card, width ? { width } : styles.fill]} testID={`product-card-${id}`}>
...
const styles = StyleSheet.create({
  ...
  add: { minHeight: 44, minWidth: 62, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, backgroundColor: colors.limeSoft, borderRadius: 10 }, addText: { fontSize: 11, color: colors.forest, fontWeight: '800' }, stepper: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.limeSoft, borderRadius: 10 }, step: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }, qty: { fontSize: 11, fontWeight: '700', color: colors.forest }, pressed: { opacity: 0.65 },
});