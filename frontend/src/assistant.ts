import { API } from './api';
export type ChatMessage = { role: 'user' | 'assistant'; text: string; product_ids?: string[]; comparison_query?: string; search_links?: { merchant: string; url: string }[]; live_prices_available?: boolean };
export type ChatSession = { id: string; messages: ChatMessage[] };
export function streamChat(session: string, message: string, onDelta: (text: string) => void, onDone: (result: ChatMessage) => void, onError: (text: string) => void) {
  ...
  xhr.send(JSON.stringify({ session_id: session, message }));
